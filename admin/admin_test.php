...
else if(!empty($_POST['freegoods_search_value']) && isset($_POST['freegoods_sort_partner'])){
$sql="select * from products as p inner join member as m on p.no=m.no where m.partner='y'
 $sort_kind_sql and p.subject like '%$freegoods_search_value%' order by sell_count/hit desc";
}
else if(empty($_POST['freegoods_search_value']) && isset($_POST['freegoods_sort_partner'])){
  ...
  <body>
        ...
        <h1>Select Handpicked / TOTAL : <?=$total_record?></h1>
            <form action="./admin_test.php?mode=search" method="post" id="freegoods_search_form" name="freegoods_search_form">
              ...
              <div id="freegoods_search_form_div2">
                ...
                <input class="switch_check" type="checkbox" value="freegoods_sort_allow" name="freegoods_sort_allow">
    ...
    <div id="admin_member_list_div">
      ...
      <a href='admin_member.php?mode=view_freegoods'>
        <button id="admin_freegoods_button_list" type="button" name="button">VIEW HANDPICKED</button>
      </a>
    </div>
